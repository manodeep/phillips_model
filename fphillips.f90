...
!     Apply the BC
   ...
   subroutine relax1(v1, v3, s1, s3)
      ...
      real :: change, maxdiff, resid, afactor
...
!!$      close(unit=99)
      ...
      afactor = accel / ( 2.0 + 2.0*epsq + gamma )
      do iter=1,100
         ! Jacobi iteration
         maxdiff = 0.0
         change = 0.0
...
!            print*, "ITER1", iter, sqrt(change), maxdiff
   ...
   subroutine relax2(x1, x3, dt, v1, v3)
      ...
      integer :: i, j, im, ip, jm, jp
      integer :: iter
      real :: change1, change3, alpha, alphafac, alphafac3

      v1 = 0.0
      v3 = 0.0
      alpha = a*dt/dx**2
      alphafac = 1.0 / ( 2*alpha*(1.0 + epsq)  + 1.0 )
      alphafac3 = 1.0 / ( 2*alpha*(1.0 + epsq)  + 1.0 +1.5*k*dt )
      do iter=1,100
         ! Jacobi iteration
         do j=1,ny-1
            jm = j-1
...
!     Solve for the zonal mean vorticity
   ...
   subroutine diag(day, day1, zonal, s1, s3)
      use constants
      real, intent(in) :: day, day1
...
!         write(unit=*,fmt="(a,f5.1,4f15.7)") "TZ ", day, maxval(t2z), maxval(u1z), maxval(u3z), zke
         print*, "TZ ", day, maxval(t2z), maxval(u1z), maxval(u3z), zke
      else
         ...
         ! Save heights at levels 1, 3 and 4
         ! Write them in reverse so it looks more like the SH
         write(unit=1) s1(:,ny:0:-1)*f0
         write(unit=1) s3(:,ny:0:-1)*f0
         write(unit=1) (1.5*s3(:,ny:0:-1) - 0.5*s1(:,ny:0:-1))*f0
         write(unit=1) t2(:,ny:0:-1)
         write(unit=1) u1t(:,ny:1:-1), (/ (0.0,jj=1,nx) /)
         write(unit=1) u3t(:,ny:1:-1), (/ (0.0,jj=1,nx) /)
         ! Need to flip sign of meridional wind too.
         write(unit=1) -v1t(:,ny:1:-1), (/ (0.0,jj=1,nx) /)
         write(unit=1) -v3t(:,ny:1:-1), (/ (0.0,jj=1,nx) /)
         close(1)
!         write(unit=*,fmt="(a,f5.1,6f15.7)") "KE ", day, maxval(t2), maxval(u1t), maxval(u3), zke, eke, tke
         print*, "KE ", day, maxval(t2), maxval(u1t), maxval(u3t), zke, eke, tke
      end if
...
subroutine fphillips(start, dt1, dt2, day1, day2, seed, noiseamp, freq, time)

!  This is a subroutine version to be driven from Python
...
!  Total fields are denoted with suffix t and zonal means with suffix z
   ...
   real, intent(in) :: dt1, dt2, day1, day2
   integer, intent(in) :: seed
   real, intent(in) :: freq ! Freq of output
   real, intent(in) :: noiseamp
   real, intent(out) :: time
   real, dimension(nx,0:ny), save :: s1t, s3t   ! Streamfunction
   real, dimension(nx,0:ny), save :: s1tm, s3tm ! Streamfunction at tau-1
  ! Need to save this to initialise relax1.
   real, dimension(nx,0:ny), save :: s1, s3     ! Streamfunction anomalies
   real, dimension(0:ny)    :: s1z, s3z   ! Zonal mean streamfunction
   real, dimension(nx,0:ny), save :: v1t, v3t   ! Vorticity
   real, dimension(nx,0:ny) :: v1, v3     ! Vorticity anomalies
   real, dimension(0:ny)    :: v1z, v3z   ! Zonal mean vorticity
   real, dimension(nx,0:ny), save :: v1tm, v3tm ! tau - 1 values
!  Temporaries used in timestepping
   ...
   integer :: i, j, im, ip, jm, jp
   real, save    :: day
   logical, save :: zonal, addnoise = .false.
   real :: dt  ! Time step
...
!  Initialise at rest
   ...
   do 
      ...
      if ( abs ( day - nint(day) ) < 0.01 .and. .not. zonal ) then
         diag_flag = .true.
      end if
      if ( day < day1 ) then
...
!            call random_number(rand)
      ...
      if ( abs ( modulo(day,freq) - nint(modulo(day,freq)) ) < 0.01 ) then
         call diag(day, day1, zonal, s1t, s3t)
      end if
...
!      print*, "Vanom", day, maxval(abs(v1))/maxval(abs(v1z))
      ...
      day = day + dt/86400.0
      time = day
      if ( ( abs ( modulo(day,freq) ) < 0.01 .or. & 
             abs ( modulo(day,freq) - freq ) < 0.01 ) .and. .not. zonal ) then
         return
      end if
...
end subroutine fphillips

program fmain
!  Driver of phillips model
   real, parameter :: dt1 = 86400., dt2 = 3600., day1 = 131., day2 = 160.
   integer :: seed = 3
   real :: time
   real :: noiseamp = 1.0
   real :: freq = 0.25
   ...
   do 
      call fphillips(start, dt1, dt2, day1, day2, seed, noiseamp, freq, time)
      start = 0
      if ( time > day2 ) exit
   end do
